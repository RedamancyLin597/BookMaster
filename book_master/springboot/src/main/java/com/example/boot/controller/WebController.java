...
public class WebController {
    ...
    public Result nums() {
        ...
        List<Book> bookStore = bookService.list();
        Integer store = bookStore.stream().map(Book::getStore).reduce(Integer::sum).orElse(0);
        dict.set("store", store);

        List<Book> bookList = bookMapper.selectList(new QueryWrapper<Book>().eq("active", "1"));
        Integer book = bookList.size();

        List<BookBorrow> bookBorrowList = bookBorrowService.list(new QueryWrapper<BookBorrow>().eq("date", DateUtil.today()).eq("back", "否").eq("status", "通过"));
        dict.set("borrow", bookBorrowList.stream().map(BookBorrow::getNum).reduce(Integer::sum).orElse(0));
        // 未归还的图书
//        Integer borrowSum = bookBorrowList.stream().filter(bookBorrow -> bookBorrow.getStatus().equals("通过")).map(BookBorrow::getNum).reduce(Integer::sum).orElse(0);
//        dict.set("borrowSum", borrowSum);
        dict.set("book", book);
