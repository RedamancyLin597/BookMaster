...
<!--      <el-table-column prop="id" label="序号" width="80" align="center" sortable></el-table-column>-->
      <el-table-column align="center" prop="bookId" label="图书封面">
        <template v-slot="scope">
          <el-image style="width: 50px; height: 50px" :src="scope.row.bookImg"
                    :preview-src-list="[scope.row.bookImg]"></el-image>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" prop="backdate" label="预还日期"></el-table-column>
      <el-table-column align="center" label="审核" width="100" v-if="user.role === 'ADMIN'">
        <template v-slot="scope">
          <el-button type="primary" @click="handleChangeStatus(scope.row)" v-if="scope.row.status === '待审核'">审核
          </el-button>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="180" align="center" v-if="user.role === 'USER'">
        <template slot-scope="scope">
          <el-button type="primary" @click="returnBook(scope.row)" size="mini"
                     v-if="scope.row.status === '通过' && scope.row.back === '否'">归还
          </el-button>
          <el-button type="primary" @click="handleBorrow(scope.row)" size="mini"
                     v-if="scope.row.status === '通过' && scope.row.back === '否'">续借
          </el-button>
          <el-popconfirm
    ...
    <el-dialog title="信息" :visible.sync="dialogFormVisible" width="30%" :close-on-click-modal="false">
      <el-form label-width="100px" style="padding-right: 40px" :model="form" :rules="rules" ref="ruleForm">
        ...
        <el-form-item prop="backdate" label="还书日期">
          <el-date-picker v-model="form.backdate" type="date" value-format="yyyy-MM-dd"
                          placeholder="选择日期"></el-date-picker>
        </el-form-item>
    ...
    <el-dialog title="续借" :visible.sync="dialogFormVisible2" width="30%" :close-on-click-modal="false">
      <el-form label-width="100px" style="padding-right: 40px" :model="form">
        <el-form-item prop="backdate" label="归还日期">
          <el-date-picker style="width: 100%" v-model="form.newBackdate" type="date" value-format="yyyy-MM-dd"
                          placeholder="选择日期"></el-date-picker>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
